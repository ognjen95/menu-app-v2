...
import { iconMap } from './utils/nav-icons'
...
export default function DashboardLayoutClient({
  ...
  const isNavItemActive = (href: string) => {
    // Exact match for settings parent route
    if (href === '/dashboard/settings') {
      return pathname === '/dashboard/settings'
    }
    // For other routes, check exact match or prefix match (excluding overview)
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <MobileBottomNav
        items={filteredNavItems.map(item => ({ ...item, icon: iconMap[item.key] }))}
      ...
      <MoreOptionsSheet
        ...
        items={filteredSettingsItems.map(item => ({ ...item, icon: iconMap[item.key] }))}
        isNavItemActive={isNavItemActive}
        t={t}
        locale={locale}
